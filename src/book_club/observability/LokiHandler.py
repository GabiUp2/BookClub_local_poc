...
class LokiHandler(Handler):
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._worker, name="LokiHandler", daemon=True
        )
            ...
            "streams": [
                {
                    ...
                    "values": [[record.created, record.getMessage()]],
                }
            ]
        }
    ...
    def _flush(self, rows: list[dict]):
        try:
            ...
            print(
                f"[LokiHandler ERROR] Failed to flush {len(rows)} rows; Exception: {sys.exc_info()}"
            )
            pass

    def _debug_flush(self, rows):
        print(
            f"[LokiHandler DEBUG] would flush {len(rows)} rows; first row:\n{rows[0] if rows else None}"
        )
