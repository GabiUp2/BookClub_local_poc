...
def _init_otel(
    ...
    enable_otel = os.getenv("ENABLE_OTEL_TRACING", "true").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    # OTEL_EXPORTER_OTLP_ENDPOINT is the base URL; append /v1/traces for HTTP trace export
    otlp_base = os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://alloy:4318").rstrip(
        "/"
    )
    otlp_traces_endpoint = f"{otlp_base}/v1/traces"
    ...
    enable_otel_logs = os.getenv("ENABLE_OTEL_LOGS", "true").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    logger.info(
        ...
        f"env={deployment_env}, endpoint={otlp_traces_endpoint}"
    )
...
async def lifespan(app: fastapi.FastAPI):
    ...
    # OTEL initialised in Gunicorn post_fork when using gunicorn.conf.py; else here (e.g. uvicorn only)
    if os.environ.get("OTEL_INIT_IN_POST_FORK") != "1":
        _init_otel()
...
async def demo_fault_injection_middleware(request: fastapi.Request, call_next):
    ...
    if _demo_config["error_rate"] > 0 and random.random() < _demo_config["error_rate"]:
        ...
        try:
            ...
            pass
        raise HTTPException(
            status_code=500, detail="Demo fault injection: probabilistic error"
        )
...
async def set_demo_faults(config: dict):
    ...
    if app_env != "local":
        raise HTTPException(
            status_code=403, detail="Demo endpoints only available in local environment"
        )
